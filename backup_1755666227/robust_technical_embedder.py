#!/usr/bin/env python3
"""
HYBRID Technical Documentation Embedder - Updated for existing setup
...
class HybridTechnicalEmbedder:
    ...
    def _create_enhanced_qa_layer(self, documents: List[Dict]) -> Chroma:
        ...
        if Path(self.primary_db_path).exists():
            ...
            logger.info(f"🗑️ Removed existing database: {self.primary_db_path}")
                ...
                if layer_db:
                    try:
                        ...
                        if results and len(results[0].page_content) > 50:
                            logger.info(f"✅ '{query[:40]}...' found in {layer_name}")
                            success_count += 1
                            found = True
                            break
