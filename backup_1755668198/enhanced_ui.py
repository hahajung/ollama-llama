#!/usr/bin/env python3
"""
Enhanced UI with Hybrid Router - No Emojis Version
Clean interface without emojis for better compatibility.
"""
...
def main():
    ...
    with st.sidebar:
        ...
        for layer_name, layer_db in storage_layers.items():
            status = "[ONLINE]" if layer_db else "[OFFLINE]"
    ...
    **Example queries that work well:**
    ...
    if 'hybrid_chatbot' in st.session_state:
        ...
        with col2:
            primary_status = "OK" if storage_layers.get('primary_qa') else "MISSING"
            st.metric("Primary DB", primary_status)
        
        with col3:
            hybrid_status = "ACTIVE" if available_layers >= 2 else "LIMITED"
